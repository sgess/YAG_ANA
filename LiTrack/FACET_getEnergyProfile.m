...
if (getHist)
  try
    [arch_times,arch_data]=getWaveformHist(strcat(kpv,'//HIST.facet'),timerange);
  catch
...
for m=1:Nmicr
  for n=1:Nunit
    if (kstat(m,n))
      getNow=~getHist;
      if (getHist)
        query=strcat(kname{m,n},':ENLD//HIST.facet');
        try
          [tval,bval]=aidaGetHistory(query,timerange);
...
for n=2:19
  ...
  if (getHist)
    if (strcmp(micr,'LI17'))
      query='EP01:AMPL:171:VACT//HIST.facet'; % feedback
    elseif (strcmp(micr,'LI18'))
      query='EP01:AMPL:181:VACT//HIST.facet'; % feedback
    else
      query=sprintf('%s:SBST:1:PDES//HIST.facet',micr);
    end
    [tval,bval]=aidaGetHistory(query,timerange);
  else
    if (strcmp(micr,'LI17'))
...
if (kstat(8,1)) % start with LI02
  if (getHist)
    [tval,bval]=aidaGetHistory('LI09:PHAS:12:VDES//HIST.facet',timerange); % feedback
  else
...
if (kstat(8,2)) % start with LI02
  if (getHist)
    [tval,bval]=aidaGetHistory('LI09:PHAS:22:VDES//HIST.facet',timerange); % feedback
  else
...
fudg=[];
if (getHist)
  query=sprintf('VX00:LEMG:%d:FUDG//HIST.facet',LEMG);
  try
    [arch_times,arch_data]=getWaveformHist(query,timerange);
    bval=arch_data;
